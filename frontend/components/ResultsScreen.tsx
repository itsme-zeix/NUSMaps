...
import { Pressable, StyleSheet,SafeAreaView, ScrollView, Text, TextInput, View, ViewStyle, TextStyle, ImageStyle, Image } from "react-native";
...
const resultCard:React.FC<ResultObject> = ({departureTiming, arrivalTiming, travelTime, types}) => {
    ...
    <View style={ styles.resultCard }>
      <View style = { styles.icons }>
        {types.map((icon, index) => (
            <Image key = {index} source = {iconList[icon as keyof IconCatalog]} style = {{flexDirection: "row", alignItems: "center"}}/>
        ))}
        <Text>{departureTiming} - {arrivalTiming}</Text>
      </View>
...
//           <Text style={{ color: "626262", fontSize: 10 }}> { busStop.distanceAway } </Text>
//         </View>
...
const ResultScreen: React.FC<ResultObject & {isVisible: boolean, setIsVisible: (isVisible: boolean) => void}> = ({origin, destination, departureTiming, arrivalTiming, travelTime, types, isVisible, setIsVisible}) => {
  if (isVisible) {
    return (
      <SafeAreaView>
        <ScrollView style = {{flex : 1}}>
            <Modal isVisible = {isVisible} animationInTiming = { 200 } 
            onBackdropPress = {() => setIsVisible(false)}
            onBackButtonPress = {() => setIsVisible(false)}
            hideModalContentWhileAnimating = { true }
            backdropColor="green" style = {styles.resultContainer}
            >
              <View style = {{flex: 1, direction:"ltr", alignItems:"center" }}>
                {resultCard({origin, destination, departureTiming, arrivalTiming, travelTime, types})}
              </View>
            </Modal>
        </ScrollView>
      </SafeAreaView>
